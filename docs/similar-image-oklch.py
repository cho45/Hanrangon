...
for mode_idx, mode in enumerate(modes):
    ...
    for bit_pos in range(64):
        ...
        # 個別のビットの色
        dot_color = oklch_to_rgb(
            lightness_steps[li_val], chroma_steps[ci_val], hue_steps[hi_val]
        )
        ax.plot(
            px,
            py,
...
plt.suptitle(
    "全64ビットのスキャンパス比較: 3次元空間(L,H,C)の埋め方\n"
    "(各マス内の2つの点は 彩度 C1:鮮やか と C0:落ち着いた色 を表す)",
    fontsize=18,
    y=1.02,
)
plt.tight_layout()
