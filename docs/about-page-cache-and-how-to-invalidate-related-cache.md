...
1.  **キャプチャ**: `Gzip` ミドルウェアの後に配置。ハンドラが出力する非圧縮 HTML をキャプチャし、URL パスをキーとして保存する。
2.  **配信**: キャッシュヒット時は保存された非圧縮データを返却。後続の `Gzip` ミドルウェアにより、リクエストに応じた適切な圧縮が行われる。
3.  **可視化**: レスポンスヘッダー `X-Cache: HIT` または `MISS` を付与する。
