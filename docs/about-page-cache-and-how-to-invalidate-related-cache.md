...
1.  **キャプチャと同期圧縮**: キャッシュミス時、ハンドラが出力する非圧縮 HTML をキャプチャしつつ、クライアントが Gzip 対応であればその場で同期的に圧縮して送信する。これにより、Echo 標準の Gzip ミドルウェアを使用せずとも、ユーザーへのレスポンスは常に圧縮される。
2.  **非同期保存と容量節約**: レスポンス送信後、バックグラウンドで Gzip 圧縮版 (`:gzip`) を作成・保存し、不要になった非圧縮版 (`:raw`) を削除してストレージ容量を節約する。
...
5.  **並行性と終了処理**:
    - `singleflight` を使用し、同一 URL に対する複数の同時リクエストが発生しても、非同期圧縮処理が重複して走らないように制御している。
    - サーバ終了時には `sync.WaitGroup` により、実行中の全ての非同期圧縮タスクの完了を待機してから DB をクローズする。
6.  **可視化**: レスポンスヘッダー `X-Cache: HIT-GZ`, `HIT-RAW`, `MISS` 等を付与する。
